...
class linear_set {
 ...
 public:
   ...
   /// @{
   linear_set() = default;
   [[maybe_unused]] linear_set(std::initializer_list<value_type> init_list) {
       linear_set::insert(init_list.begin(), init_list.end());
   }

   template <typename Iterator> [[maybe_unused]] linear_set(Iterator first1, Iterator last1) {
       linear_set::insert(first1, last1);
   }
   /// @}

   /// @returns Value-to-key converter.
   [[maybe_unused]] key_from_value key_extractor() const { return key_from_value(); }
 ...
 private:
   ...
   [[maybe_unused]] iterator find_value(const value_type& value) {
       return find(key_from_value()(value));
   }
   /// @}
