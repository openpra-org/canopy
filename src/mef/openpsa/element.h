...
#include "mef/openpsa/linear_set.h"
#include "mef/openpsa/multi_index.h"
...
class ContainerElement {
 ...
 protected:
   ...
   [[nodiscard]] const Element* container() const { return container_; }
...
class Attribute {
 public:
   ...
   void name(std::string name) {
       if (name.empty())
           throw LogicError("Attribute name cannot be empty.");
       name_ = std::move(name);
   }
   ...
   void value(std::string value) {
       if (value.empty())
           throw LogicError("Attribute value cannot be empty.");
...
class Element : public ContainerElement, private boost::noncopyable {
   ...
   struct [[maybe_unused]] AttributeKey {
 ...
 public:
   ...
   using AttributeMap = linear_set<Attribute, AttributeKey>;
   ...
   explicit Element(std::string name) { Element::name(std::move(name)); }

   /// @returns The original name.
   [[nodiscard]] const std::string& name() const { return name_; }

   /// @returns The string view to the name for table keys.
   [[nodiscard]] std::string_view name_view() const { return name_; }
   ...
   [[maybe_unused]] void AddAttribute(Attribute attr) {
       if (!attributes_.insert(std::move(attr)).second) {
           throw ValidityError("Duplicate attribute: "+name_+" "+attr.name());
       }
   }
   ...
   /// @warning Pointers or references
   ///          to existing attributes may get invalidated.
   [[maybe_unused]] void SetAttribute(Attribute attr) noexcept {
   ...
   [[nodiscard]] const Attribute* GetAttribute(std::string_view name) const noexcept {
   ...
   [[maybe_unused]] std::optional<Attribute> RemoveAttribute(std::string_view name) noexcept {
...
class Role {
 public:
   ...
   explicit Role(RoleSpecifier role = RoleSpecifier::kPublic,
                 std::string base_path = "") : kBasePath_(std::move(base_path)), kRole_(role) {
       if (!kBasePath_.empty() &&
   ...
   [[nodiscard]] RoleSpecifier role() const { return kRole_; }

   /// @returns The base path containing ancestor container names.
   [[nodiscard]] const std::string& base_path() const { return kBasePath_; }
...
class Id : public Element, public Role {
 public:
   ...
   explicit Id(std::string name, std::string base_path = "",
               RoleSpecifier role = RoleSpecifier::kPublic) : Element(std::move(name)),
   ...
   [[nodiscard]] std::string_view id_view() const { return id(); }

   /// @returns The string view to the unique full path for a table key.
   [[maybe_unused]] [[nodiscard]] std::string_view full_path() const { return full_path_; }
   ...
   [[maybe_unused]] static const std::string& unique_name(const Id& element) {
       return element.id();
   }
   /// @}
...
class Container {
 public:
   ...
   [[maybe_unused]] Pointer Remove(T* element) {
       ...
       try {
           if (it == table_.end())
               throw(UndefinedElement());
           if (&**it != element)
               throw(LogicError("Duplicate element with different address."));
       ...
       return extract(it, &table_);  // no-throw.
   }
...
template <class T, class... Ts>
using MultiContainer [[maybe_unused]] = Composite<Container<T, Ts>...>;
...
class NodeMark {
 public:
   ...
   enum Mark : std::uint8_t {
       ...
       kTemporary [[maybe_unused]],
       kPermanent [[maybe_unused]]
   };

   /// @returns The mark of this node.
   [[nodiscard]] Mark mark() const { return mark_; }
...
class Usage {
 public:
   ...
   [[maybe_unused]] [[nodiscard]] bool usage() const { return usage_; }

   /// @param[in] usage  The usage state of the element in a model.
   [[maybe_unused]] void usage(bool usage) { usage_ = usage; }
