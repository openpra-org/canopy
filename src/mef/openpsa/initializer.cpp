...
#include "mef/openpsa/cycle.h"
#include "mef/openpsa/env.h"
#include "mef/openpsa/error.h"
#include "mef/openpsa/algorithm.h"
#include "mef/openpsa/find_iterator.h"

#include "mef/openpsa/expr/boolean.h"
#include "mef/openpsa/expr/conditional.h"
#include "mef/openpsa/expr/exponential.h"
#include "mef/openpsa/expr/extern.h"
#include "mef/openpsa/expr/numerical.h"
#include "mef/openpsa/expr/random_deviate.h"
   ...
   for (const auto& path_str : xml_files) {
       ...
       if (path_str.find('*') != std::string::npos || path_str.find('?') != std::string::npos) {
           ...
           try {
               ...
               throw IOError("Error accessing directory");
               //<< boost::errinfo_file_name(dir.string());
           }
       } else {
...
void Initializer::CheckFileExistence(const std::vector<std::string> &xml_files) {
   for (auto &xml_file: xml_files) {
       if (!boost::filesystem::exists(xml_file)) {
           throw(IOError("Input file doesn't exist: "+xml_file));
       }
   }
}
...
void Initializer::CheckDuplicateFiles(
   ...
   for (auto& xml_file : xml_files) {
       ...
       if (!is_unique)
           throw(IOError("Duplicate input file: "+ *it));
   }
}
...
void Initializer::Define(const io::xml::Element& event_node,
   ...
   if (!expressions.empty()) {
       ...
       throw(ValidityError("The basic event "+std::string(basic_event->id()) +" at line "+ std::to_string(event_node.line())+" does not have an expression."));
   }
}
...
Instruction* Initializer::GetInstruction(const io::xml::Element& xml_element) {
   ...
   if (node_name == "set-house-event") {
       ...
       if (!model_->house_events().count(name)) {
           throw(UndefinedElement("[line]: "+std::to_string(xml_element.line())+", house event: "+std::string(name)));
       }
...
Expression* Initializer::GetParameter(const std::string_view& expr_type,
   ...
   auto check_units = [&expr_element](const auto& parameter) {
       ...
       const char* param_unit = kUnitsToString[parameter.unit()];
...
T* Initializer::GetEntity(std::string_view entity_reference,
   ...
   if (!base_path.empty()) {  // Check the local scope.
       ...
       if (auto it = find(path_container, full_path))
           return &*it;
   }

   auto at = [&entity_reference, &base_path](const auto& reference_container) {
       if (auto it = find(reference_container, entity_reference))
           return &*it;
...
#define GET_EVENT(gates, basic_events, house_events, path_reference) \
 do {                                                               \
   if (auto it = find(gates, path_reference))                  \
     return &*it;                                                   \
   if (auto it = find(basic_events, path_reference))           \
     return &*it;                                                   \
   if (auto it = find(house_events, path_reference))           \
     return &*it;                                                   \
 } while (false)
   ...
   struct CheckOrder {
       ...
       void operator()(Fork* fork) const {
           if (functional_event.order() == fork->functional_event().order()) {
               ...
               throw(ValidityError("Functional event " +
   ...
   for (const Substitution& origin : substitutions) {
       ...
       for (const Substitution& substitution : substitutions) {
           ...
           auto in_hypothesis = [&substitution](const BasicEvent* source) {
               return any_of(substitution.hypothesis().args(),
...
//                   << errinfo_element(origin.name(), "substitution");
           if (any_of(origin.source(), in_hypothesis))
               throw(
...
void Initializer::EnsureNoCcfSubstitutions() {
   ...
   auto is_ccf = [](const Substitution& substitution) {
       if (any_of(substitution.hypothesis().args(),
       ...
       if (any_of(substitution.source(), std::mem_fn(&BasicEvent::HasCcf)))
           return true;
   ...
   for (const Substitution& substitution : substitutions) {
       if (is_ccf(substitution))
           throw(ValidityError("Non-declarative substitution '" +
                                     substitution.name() +
...
void Initializer::EnsureSubstitutionsWithApproximations() {
   if (settings_.approximation() != Approximation::kNone)
       return;

   if (any_of(model_->substitutions(),
