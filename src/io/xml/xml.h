...
std::enable_if_t<std::is_arithmetic_v<T>, T> to(const std::string_view& value) {
   if constexpr (std::is_same_v<T, int>) {
       ...
       if (len != value.size() || ret > std::numeric_limits<int>::max() ||
           ...
           throw ValidityError("Failed to interpret value to int"+std::string(value));
       }
       return ret;
       ...
       if (len != value.size() || ret == HUGE_VAL || ret == -HUGE_VAL) {
           throw ValidityError("Failed to interpret value to double"+std::string(value));
       }
       return ret;
       ...
       if (value == "false" || value == "0")
           return false;
       throw ValidityError("Failed to interpret value to bool"+std::string(value));
   }
}
...
class Element {
 public:
     ...
     public:
       ...
       class iterator
         ...
         private:
           ...
           [[nodiscard]] Element dereference() const { return Element(element_); }
           /// @}
       ...
       [[nodiscard]] iterator cbegin() const { return begin_; }
       [[nodiscard]] static iterator cend() { return iterator(); }
       /// @}
   ...
   [[nodiscard]] const char* filename() const { return detail::from_utf8(element_->doc->URL); }

   /// @returns The line number of the element.
   [[nodiscard]] int line() const { return XML_GET_LINE(to_node()); }

   /// @returns The name of the XML element.
   ///
   /// @pre The element has a name.
   [[nodiscard]] std::string_view name() const { return detail::from_utf8(element_->name); }
   ...
   attribute(const char* name) const {
       ...
       try {
           ...
           std::string msg = std::string(Element::name()) +", "+std::to_string(line())+","+filename();
           throw ValidityError(msg);
       }
   }
   ...
   [[nodiscard]] Range children() const { return Range(element_->children); }
...
class Document {
 public:
   ...
   [[maybe_unused]] explicit Document(const std::string& file_path,
   ...
   /// @{
   [[nodiscard]] const xmlDoc* get() const { return doc_.get(); }
...
class Validator {
 public:
   ...
   [[maybe_unused]] explicit Validator(const std::string& rng_file);
   ...
   void validate(const Document& doc) {
       ...
       if (ret != 0) {
           throw detail::GetError<ValidityError>();
       }
   }
