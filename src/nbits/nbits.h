...
#include <AdaptiveCpp/hipSYCL/sycl/buffer.hpp>
#include <cassert>
#include <climits>
#include <vector>
...
namespace canopy {
    ...
    /**
     * @brief Concept to ensure the derived class implements the `any()` method.
     *
     * This concept checks that the type `T` has member function `any()` that returns `bool`.
     *
     * @tparam T The type to check.
     */
    template<typename T>
    concept RequiresAny = requires(const T& t) {
        { t.any() } -> std::convertible_to<bool>;
    };

    /**
     * @brief Concept to ensure the derived class implements the `all()` method.
     *
     * This concept checks that the type `T` has member function `all()` that returns `bool`.
     *
     * @tparam T The type to check.
     */
    template<typename T>
    concept RequiresAll = requires(const T& t) {
        { t.all() } -> std::convertible_to<bool>;
    };
    ...
    public:
        ...
        static bool any(const DerivedType& bits) requires RequiresAny<DerivedType> {
            return bits->any();
        }

        template<typename DerivedType>
        static bool all(const DerivedType& bits) requires RequiresAll<DerivedType> {
            return bits->all();
        }
    };
}
