        ...
        host("NVIDIA GPU Tests") {
            ...
            shellScript("ctest") {
                ...
                content = """
                          trap 'docker rmi "$remote:ci-{{ run:number }}-{{ branchSlug }}"' EXIT
                          docker pull "$remote:ci-{{ run:number }}-{{ branchSlug }}"
                          docker run --rm "$remote:ci-{{ run:number }}-{{ branchSlug }}" ctest --verbose --output-on-failure
                          """
            }
            ...
            shellScript("run") {
                ...
                content = """
                          trap 'docker rmi "$remote:ci-{{ run:number }}-{{ branchSlug }}"' EXIT
                          docker pull "$remote:ci-{{ run:number }}-{{ branchSlug }}"
                          docker run --rm "$remote:ci-{{ run:number }}-{{ branchSlug }}" ./src/bool/bool
                          """
            }
        }
        ...
        host("OpenMP CPU Tests") {
            ...
            shellScript("ctest") {
                ...
                content = """
                          trap 'docker rmi "$remote:ci-{{ run:number }}-{{ branchSlug }}"' EXIT
                          docker pull "$remote:ci-{{ run:number }}-{{ branchSlug }}"
                          docker run --rm "$remote:ci-{{ run:number }}-{{ branchSlug }}" ctest --verbose --output-on-failure || true
                          """
            }
            ...
            shellScript("run") {
                ...
                content = """
                          trap 'docker rmi "$remote:ci-{{ run:number }}-{{ branchSlug }}"' EXIT
                          docker pull "$remote:ci-{{ run:number }}-{{ branchSlug }}"
                          docker run --rm -e ACPP_VISIBILITY_MASK=omp "$remote:ci-{{ run:number }}-{{ branchSlug }}" ./src/bool/bool
                          """
            }
        }
        ...
        host("OpenCL AMD CPU Tests") {
            ...
            shellScript("run") {
                ...
                content = """
                          trap 'docker rmi "$remote:ci-{{ run:number }}-{{ branchSlug }}"' EXIT
                          docker pull "$remote:ci-{{ run:number }}-{{ branchSlug }}"
                          docker run --rm -e ACPP_VISIBILITY_MASK=ocl "$remote:ci-{{ run:number }}-{{ branchSlug }}" ./src/bool/bool
                          """
            }
        }
